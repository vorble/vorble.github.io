"use strict";
function fightCalculatePhysicalDamage(attacker, defender) {
    // Raw attack is determined by attacker's physical weapon level and is increased by 1% for each
    // point of str and dex.
    const atkabs = Math.round(Math.abs(attacker.weapon.physical) * (1.0 + (attacker.str + attacker.dex) * 0.01));
    // Raw defense is determined by defender's physical armor level and is increased by 0.8% for each
    // point of str, dex, and con.
    const defabs = Math.round(Math.abs(defender.armor.physical) * (1.0 + (defender.str + defender.dex + defender.con) * 0.008));
...
function fightCalculateMagicalDamage(attacker, defender) {
    let atkabs = Math.round(Math.abs(attacker.weapon.magical) * (1.0 + (attacker.int + attacker.wis) * 0.01));
    ...
    const defabs = Math.round(Math.abs(defender.armor.magical) * (1.0 + (defender.int + defender.wis + defender.con) * 0.008));
    const mitigation = defabs > atkabs ? defabs - atkabs : 0;
    const damage = atkabs > mitigation ? atkabs - mitigation : 0;
    return damage;
}
function fightCalculateElementalDamage(attacker, defender) {
    let atkabs = Math.round(Math.abs(attacker.weapon.elemental) * (1.0 + (attacker.con) * 0.02));
    const defabs = Math.round(Math.abs(defender.armor.elemental) * (1.0 + (defender.con) * 0.02));
    const mitigation = defabs > atkabs ? defabs - atkabs : 0;
    let damage = atkabs > mitigation ? atkabs - mitigation : 0;
...
function fightCalculateAttack(attacker, defender) {
