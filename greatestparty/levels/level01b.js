    ...
    newTown: (game) => {
        ...
        town.events = [
            {
                name: 'A Faux Pas',
                weight: 3,
                predicate: (game) => !game.party.status.angeredGods.active,
                action: (game) => {
                ...
                action: (game) => {
                    ...
                    if (rollDie(20) + modLinear(game.party.int, 10) <= 15) { //Being smart helps you trap the outlaws
                        ...
                        if (rollDie(20) + modLinear(game.party.str, 10) <= 10 && rollDie(20) + modLinear(game.party.dex, 10) <= 10) {
                            ...
                            if (gold > game.party.gold) {
                                game.receiveGold(-game.party.gold);
                            }
                            else {
                                game.receiveGold(-gold);
                            }
                            ...
                            for (let i = rollDie(3); i >= 0; --i) {
                                loot(game);
                            } // at least you walk off with some equipment
                        }
                    }
        ...
        town.quests = [
            ...
            },
            {
                name: 'Blazing Shards',
                weight: 2,
                predicate: (game) => !townState.blazingShardsDone,
                action: (game) => {
            ...
            },
            {
                ...
                weight: 2,
                predicate: (game) => townState.blazingShardsDone && !townState.cliffsDone,
                action: (game) => {
            ...
            },
            {
                name: 'Summoning Pyre',
                weight: 2,
                predicate: (game) => townState.cliffsDone && !townState.pyreDone,
                action: (game) => {
            ...
            },
            {
                name: 'Scaling the Cliffs',
                weight: 2,
                predicate: (game) => townState.pyreDone && !townState.snuffDone,
                action: (game) => {
        ...
        town.enemies = [
            {
                weight: 1,
                predicate: (game) => townState.blazingShardsDone && !townState.snuffDone,
                roll: (game) => {
                    return Object.assign(Object.assign({ name: 'Opposing Party Member', health: 28 }, OPPOSITION_STATS), { weapon: {
                            physical: 5,
                            magical: -5,
                            elemental: 3,
                        }, armor: {
...
//# sourceMappingURL=level01b.js.map